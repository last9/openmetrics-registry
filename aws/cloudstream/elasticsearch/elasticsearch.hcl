scraper aws_elasticsearch_cloudstream module {
  frequency  = 60
  lookback   = 600
  timeout    = 30
  resolution = 60
  ...
  gauge "kibana_healthy_nodes" {
    source promql "kibana_healthy_nodes" {
      query = <<-EOF
        min by(ClientId, DomainName) (amazonaws_com_AWS_ES_KibanaHealthyNodes{ClientId=~'${join("|", resources.all.ClientId)}', DomainName=~'${join("|", resources.all.DomainName)}'})
      EOF
    }
  }
  ...
  gauge "master_cpu" {
    source promql "master_cpu" {
      query = <<-EOF
        avg by(ClientId, DomainName) (amazonaws_com_AWS_ES_MasterCPUUtilization{ClientId=~'${join("|", resources.all.ClientId)}', DomainName=~'${join("|", resources.all.DomainName)}'})
      EOF
    }
  }
  ...
  gauge "master_jvm_memory_pressure" {
    source promql "master_jvm_memory_pressure" {
      query = <<-EOF
        max by(ClientId, DomainName) (amazonaws_com_AWS_ES_MasterJVMMemoryPressure{ClientId=~'${join("|", resources.all.ClientId)}', DomainName=~'${join("|", resources.all.DomainName)}'})
      EOF
    }
  }
}
